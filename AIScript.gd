...
@export var AIMovementSpeed = -80 # How fast can the AI move
...
#Vector
var speed = Vector2()  # Current velocity of the AI
...
@onready var player = get_parent().get_node("player")  # Target to focus on (currently its Ryu )
...
func _move_AI():
	speed = Vector2()
	speed.x += moveDir * AIMovementSpeed
	velocity.x = speed.x
	move_and_slide()
